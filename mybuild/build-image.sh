...
INVESTMENT_SSR_IMAGE_REGISTRY="${INVESTMENT_SSR_IMAGE_REGISTRY:-harbor.sunmoonai.com}"
INVESTMENT_SSR_IMAGE_PROJECT="${INVESTMENT_SSR_IMAGE_PROJECT:-app-images}"
...
build_image() {
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        log_info "镜像信息:"
        $RUNTIME_CMD images "${INVESTMENT_SSR_IMAGE}:${INVESTMENT_SSR_TAG}"
...
push_image() {
    ...
    # 构建完整镜像名称
    local push_registry
    push_registry="$(resolve_harbor_registry_for_push "${INVESTMENT_SSR_IMAGE_REGISTRY:-harbor.sunmoonai.com}")"
    INVESTMENT_SSR_FULL_IMAGE_NAME="${push_registry}/${INVESTMENT_SSR_IMAGE_PROJECT}/${INVESTMENT_SSR_IMAGE}:${INVESTMENT_SSR_TAG}"
    ...
    # 推送镜像
    if push_image_with_harbor_verify "$RUNTIME_CMD" "$INVESTMENT_SSR_FULL_IMAGE_NAME"; then
