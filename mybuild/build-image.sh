...
RESEARCH_SSR_IMAGE="${RESEARCH_SSR_IMAGE:-research-web-frontend}"
RESEARCH_SSR_TAG="${RESEARCH_SSR_TAG:-1.0.0}"

# 镜像仓库配置（从 build.conf 读取）
RESEARCH_SSR_IMAGE_REGISTRY="${RESEARCH_SSR_IMAGE_REGISTRY:-harbor.sunmoonai.com}"
RESEARCH_SSR_IMAGE_PROJECT="${RESEARCH_SSR_IMAGE_PROJECT:-app-images}"
...
if [[ "$1" == "--tag" && -n "$2" ]]; then
    CUSTOM_TAG="$2"
    RESEARCH_SSR_TAG="$CUSTOM_TAG"
    log_info "使用自定义标签: $RESEARCH_SSR_TAG"
fi
...
build_image() {
    log_info "开始构建镜像..."
    log_info "镜像名称: ${RESEARCH_SSR_IMAGE}:${RESEARCH_SSR_TAG}"
    ...
    if [ $? -eq 0 ]; then
        log_success "镜像构建完成: ${RESEARCH_SSR_IMAGE}:${RESEARCH_SSR_TAG}"
        echo ""
        log_info "镜像信息:"
        $RUNTIME_CMD images "${RESEARCH_SSR_IMAGE}:${RESEARCH_SSR_TAG}"
...
push_image() {
    ...
    # 构建完整镜像名称
    local push_registry
    push_registry="$(resolve_harbor_registry_for_push "${RESEARCH_SSR_IMAGE_REGISTRY:-harbor.sunmoonai.com}")"
    RESEARCH_SSR_FULL_IMAGE_NAME="${push_registry}/${RESEARCH_SSR_IMAGE_PROJECT}/${RESEARCH_SSR_IMAGE}:${RESEARCH_SSR_TAG}"
    
    log_info "完整镜像名称: $RESEARCH_SSR_FULL_IMAGE_NAME"
    
    # 检查本地镜像是否存在
    if ! $RUNTIME_CMD images | grep -q "${RESEARCH_SSR_IMAGE}.*${RESEARCH_SSR_TAG}"; then
        log_error "本地镜像不存在: ${RESEARCH_SSR_IMAGE}:${RESEARCH_SSR_TAG}"
        log_info "请先构建镜像: ./build-image.sh"
        exit 1
    fi
    
    # 标记镜像
    $RUNTIME_CMD tag "${RESEARCH_SSR_IMAGE}:${RESEARCH_SSR_TAG}" "$RESEARCH_SSR_FULL_IMAGE_NAME"
    
    # 推送镜像
    if push_image_with_harbor_verify "$RUNTIME_CMD" "$RESEARCH_SSR_FULL_IMAGE_NAME"; then
        log_success "✅ 镜像推送成功: $RESEARCH_SSR_FULL_IMAGE_NAME"
    else
        log_error "❌ 镜像推送失败: $RESEARCH_SSR_FULL_IMAGE_NAME"
        log_info "请检查："
        log_info "  1. 镜像仓库配置是否正确"
        if [[ "$RUNTIME_CMD" == "docker" ]]; then
            log_info "  2. 是否已登录镜像仓库（docker login ${RESEARCH_SSR_IMAGE_REGISTRY}）"
        else
            log_info "  2. 是否已登录镜像仓库（sudo nerdctl login ${RESEARCH_SSR_IMAGE_REGISTRY}）"
        fi
        log_info "  3. 网络连接是否正常"
        exit 1
    fi
...
# 主函数
main() {
    log_info "Research App SSR 镜像构建脚本启动"
