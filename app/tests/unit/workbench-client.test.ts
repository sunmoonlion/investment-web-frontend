...
function event(cursor: number, type = 'turn/requested') {
  return { id: `00000000-0000-5000-8000-0000000000${String(cursor).padStart(2, '0')}`, session_id: sid, cursor, kind: 'session', type, payload: { text: 'hi' } }
}
...
describe('workbench client', () => {
  it('sends mutations as same-origin POST with CSRF and correlation headers', async () => {
    ...
    await expect(createSession({ environment_id: sid, sandbox_id: sid, project_root: '/r' }, csrf, fetchImpl)).resolves.toBe(sid)
  ...
  it('generates a request id per turn so retries are idempotent', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValue(json({ request_id: 'x', command_id: sid, cursor: 3 }))
  ...
  it('surfaces the problem+json top-level code', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValue(json({ code: 'wheel_not_held', status: 409, detail: 'advisor drives' }, 409))
    await expect(startTurn(sid, 'x', csrf, fetchImpl)).rejects.toEqual(expect.objectContaining<Partial<WorkbenchClientError>>({ code: 'wheel_not_held', status: 409 }))
  })

  it('fails closed on a page that breaks the contract', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValue(json({ contract_version: 2, events: [], next_cursor: 0 }))
  ...
  it('reads an events page and forwards the cursor', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValue(json({ contract_version: 1, events: [event(5), event(6)], next_cursor: 6 }))
  ...
  it('normalises budget amounts to strings in a task view', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValue(
      json({
        task: { id: tid, session_id: sid, profile_id: 'SMOKE', state: 'RUNNING', waiting_reason: null, current_step: 1, budget: { currency: 'CNY', limit: 5, reserved: '0', used: 1.32 } },
        attempts: [],
        artifacts: [],
      }),
    )
  ...
  it('posts handover and interaction responses verbatim', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValueOnce(json({ task_id: tid, state: 'RECEIVED', created: true })).mockResolvedValueOnce(json({ interaction_id: sid }))
    await handover(sid, { idempotency_key: 'k', profile_id: 'SMOKE', original_input: { text: 'q' }, budget_limit: '5' }, csrf, fetchImpl)
    await respondInteraction(sid, { token: 'tok-1234567890123456', decision: 'allow' }, csrf, fetchImpl)
    expect(String(fetchImpl.mock.calls[0][0])).toBe(`/api/workbench/sessions/${sid}/handover`)
    expect(JSON.parse(String(fetchImpl.mock.calls[1][1]?.body))).toEqual({ token: 'tok-1234567890123456', decision: 'allow' })
  })
